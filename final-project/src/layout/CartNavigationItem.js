...
export const CartNavigationItem = ({ pathname }) => {
  const condition = pathname === "/cart";
  const [color, setColor] = useState(
    condition ? "var(--primary)" : "var(--light-text)"
  );

  return (
    <Box onClick={moveToCart}
      sx={{
        display: "flex",
        ...
        cursor: "pointer",
        color: condition ? "var(--primary)" : "var(--light-text)",
